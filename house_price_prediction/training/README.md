...
directory of the corresponding component, for example, model training notebooks in `house_price_prediction/training/notebooks`, 
batch inference notebook in `house_price_prediction/deployment/batch_inference/notebooks`.
...
Address TODOs in the recipe configs under `house_price_prediction/training/recipe.yaml`, `house_price_prediction/training/profiles/databricks-dev.yaml`,
and `house_price_prediction/training/profiles/local.yaml`, specifying configs such as the training dataset path(s) to use when developing
...
After configuring your recipe, you can iterate on and test ML code under ``house_price_prediction/training/steps``.
We expect most development to take place in the abovementioned YAML config files and
`house_price_prediction/training/steps/train.py` (model training logic).

## Iterating on ML code

### Deploy ML code and resources to dev workspace using Bundles

Refer to [Local development and dev workspace](../../house_price_prediction/databricks-resources/README.md#local-development-and-dev-workspace) 
...
* Run `databricks configure --profile house-price-prediction-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile house-price-prediction-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)

#### Running code on Databricks
You can iterate on ML code by running the provided `house_price_prediction/training/notebooks/Train.py` notebook on Databricks using
[Repos](https://learn.microsoft.com/azure/databricks/repos/index). This notebook drives execution of
the ML code defined under ``house_price_prediction/training/steps``. You can use multiple browser tabs to edit
...
#### Prerequisites
* Python 3.8+
* Install model training and test dependencies via `pip install -I -r house_price_prediction/requirements.txt -r test-requirements.txt` from project root directory.
