...
The databricks CLI bundles top level is defined by file `house_price_prediction/bundle.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`house_price_prediction/databricks-resources/model-workflow-resource.yml`)
 - batch inference workflow (`house_price_prediction/databricks-resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`house_price_prediction/databricks-resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`house_price_prediction/databricks-resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`house_price_prediction/databricks-resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` environment is used by ML engineers to deploy ML resources to development workspace with `dev` environment configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-house-price-prediction-model     | /dev-house-price-prediction-experiment     |
| staging     | The `staging` environment is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` environment config.                                                             | staging workspace    | staging-house-price-prediction-model | /staging-house-price-prediction-experiment |
| prod        | The `prod` environment is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` environment config.                                                                      | prod workspace       | prod-house-price-prediction-model    | /prod-house-price-prediction-experiment    |
| test        | The `test` environment is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` environment. | staging workspace    | test-house-price-prediction-model    | /test-house-price-prediction-experiment    |
...
2. Complete the `TODO` in `house_price_prediction/bundle.yml` to add the dev workspace URI under `environments.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `house_price_prediction/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `house_price_prediction/databricks-resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.environment}-house-price-prediction-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.environment}-house-price-prediction-batch-inference-job`
that runs the notebook under `house_price_prediction/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.environment}-house-price-prediction-batch-inference-job`. 
...
`test-house-price-prediction-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-house-price-prediction-batch-inference-job`.
